...
def store_download_data(download_id: str, table_markdown: str, query: str):
    ...
    # ✅ CLEANUP: Remove old entries (keep only last 100)
    if len(download_storage) > 100:
        oldest_keys = sorted(download_storage.keys(), 
                           key=lambda k: download_storage[k]['created_at'])[:20]
        for key in oldest_keys:
            del download_storage[key]
...
async def download_modified_table(
    ...
    try:
        # ✅ STEP 1: Get table data from storage
        table_data = download_storage.get(download_id)
        
        if not table_data:
            # ✅ FALLBACK: Try to get from recent chat messages
            table_data = await get_table_from_recent_chats(download_id)
        
        if not table_data:
            raise HTTPException(status_code=404, detail="Download not found or expired")
        
        # ✅ STEP 2: Extract table content
        table_markdown = table_data.get('table_markdown', '')
        original_query = table_data.get('query', query)
        
        if not table_markdown:
            raise HTTPException(status_code=404, detail="No table content found")
        
        # ✅ STEP 3: Convert based on format preference
        ...
        # ✅ STEP 4: Return file with proper headers
        headers = {
...
async def send_message(
    ...
    try:
        ...
        # ✅ ENHANCED: Store download data and add frontend helpers
        if (result.get("success") and 
            ...
            # Store download data if table modification
            if (metadata.get("is_modification") and metadata.get("download_ready")):
                store_download_data(
                    download_id=metadata["download_id"],
                    table_markdown=metadata["new_table_markdown"],
                    query=request.message
                )
            
            # ✅ ADD: Frontend integration helpers
            result["frontend"] = {
                "response_type": metadata.get("response_type", "text"),
                "show_download_button": metadata.get("show_download_button", False),
                "download_id": metadata.get("download_id"),
                "has_image": metadata.get("has_image", False),
                "modified_table": metadata.get("modified_table_markdown")
            }
